...
local function gen_day_pattern(time_delta)
	...
	for _, v in ipairs(conf.pattern_day) do
		local cp = v:gsub("年", "Y")
			:gsub("月", "M")
			:gsub("{(year)}(.?){(month)}(.?){(day)}", "{%1~}" .. "%2" .. "{%3~}" .. "%4" .. "{%5" .. offset_num .. "}")
			:gsub("Y", "年")
			:gsub("M", "月")
...
function T.func(input, seg, env)
	...
	-- 日期
	if
		(seg:has_tag("date") or (input == "date") or (input == "/wd") or (input == "rq"))
	...
	-- 星期
	if (seg:has_tag("week") or input == "week" or input == "/wk") and (not seg:has_tag("easy_en")) then
		local tip = "〔星期〕"
		segment.prompt = tip
	...
	-- 时间
	if (seg:has_tag("time") or input == "time" or input == "/wt") and (not seg:has_tag("easy_en")) then
		local tip = "〔时间〕"
		segment.prompt = tip
	...
	-- 时间戳
	if (seg:has_tag("timestamp") or input == "timestamp" or input == "/uts") and (not seg:has_tag("easy_en")) then
		local tip = "〔时间戳〕"
		segment.prompt = tip
